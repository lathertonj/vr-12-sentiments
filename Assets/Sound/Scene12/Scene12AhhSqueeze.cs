...
public class Scene12AhhSqueeze : MonoBehaviour
{
	...
	public string[] myHighNotes;
	private string myAhhGoalVolume, mySqueezedEvent, myUnsqueezedEvent;
	private float currentVibration, goalVibration, vibrationSlew;
    ...
    void Start()
    {
        ...
        ", mySqueezedEvent, myUnsqueezedEvent, string.Join( ",", myHighNotes ) ) );
    }
    ...
    void Update()
    {
		// set ahh volume based on hand speed
		float handSpeed = myController.Velocity().magnitude;
        myChuck.SetFloat( myAhhGoalVolume, handSpeed.MapClamp( 0, 3, 0, 1 ) );

		// compute vibration
		currentVibration += vibrationSlew * ( goalVibration - currentVibration );
		myController.Vibrate( (ushort) currentVibration );
		
		// inform that squeezes happened and did not happen
